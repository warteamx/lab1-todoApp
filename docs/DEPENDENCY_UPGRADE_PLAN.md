...
| jest-expo                       | ~53.0.9      | ~53.0.27     | patch  |
...
- [ ] Server builds without errors (`npm run build` in `server/`)
...
- [ ] Client lint passes (`npm run lint` in `client/`)
- [ ] Client tests pass (`npm run test:ci` in `client/`)
